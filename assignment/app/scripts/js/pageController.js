...
function populateCards() {
	...
	document.getElementById('costaCardTitle').innerHTML = '<h3>' + cardData['costa'].title + '</h3';
	document.getElementById('costaCardDescription').innerHTML = '<p>' + cardData['costa'].description + '</p>';
	document.getElementById('costaCardImage').src = "assets/images/cards/" + cardData['costa'].image;
	document.getElementById('costaCardLink').href = "javascript:switchModelView('" + cardData['costa'].link + "')";



	document.getElementById('poweradeCardTitle').innerHTML = '<h3>' + cardData['powerade'].title + '</h3';
	document.getElementById('poweradeCardDescription').innerHTML = '<p>' + cardData['powerade'].description + '</p>';
	document.getElementById('poweradeCardImage').src = "assets/images/cards/" + cardData['powerade'].image;
	document.getElementById('poweradeCardLink').href = "javascript:switchModelView('" + cardData['powerade'].link + "')";
}
...
$(document).ready(function () {
		...
		// Hide the modal when the close button or the background is clicked
		...
		$("#contactForm").submit(function (event) {
		  ...
		  request.open("POST", "scripts/php/send_email.php");
		  request.send(formData);
		
		  request.onreadystatechange = function() {
			if (request.readyState === XMLHttpRequest.DONE) {
			  if (request.status === 200) {
				alert("Email sent successfully! Don't forget to check Spam/Junk folder.");
			  } else {
